...
export class Event {
    ...
    running;
    logger;
    constructor(wrapFn, abi, listener) {
    ...
    async filter() {
        try {
            ...
            if (latestBlock >= this.listenerBlock.startBlock) {
                const events = await this.emitContract.queryFilter(this.eventName, this.listenerBlock.startBlock, latestBlock);
                console.debug({ name: "Event:filter" }, `${this.listenerBlock.startBlock}-${latestBlock}`);
