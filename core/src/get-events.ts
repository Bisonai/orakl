...
import { IListenerBlock } from './types'
  ...
  async get_events() {
    try {
      ...
      if (latest_block >= this.listener_block.startBlock) {
        const events = await this.emit_contract.queryFilter(
          this.eventName,
          this.listener_block.startBlock,
          latest_block
        )
...
export async function get_events(
  ...
  try {
    ...
    if (latest_block >= listener_block.startBlock) {
      const events = await emit_contract.queryFilter(
        eventName,
        listener_block.startBlock,
        latest_block
      )
      //save last block here
