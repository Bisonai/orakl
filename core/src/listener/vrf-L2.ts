...
import {
  CHAIN,
  L2_CHAIN,
  L2_LISTENER_VRF_HISTORY_QUEUE_NAME,
  L2_LISTENER_VRF_LATEST_QUEUE_NAME,
  L2_LISTENER_VRF_PROCESS_EVENT_QUEUE_NAME,
  L2_VRF_LISTENER_STATE_NAME,
  L2_VRF_SERVICE_NAME,
  L2_WORKER_VRF_QUEUE_NAME
} from '../settings'
...
export async function buildListener(
  ...
  const stateName = L2_VRF_LISTENER_STATE_NAME
  const service = L2_VRF_SERVICE_NAME
  const l2Chain = L2_CHAIN
  const eventName = 'RandomWordsRequested'
  const latestQueueName = L2_LISTENER_VRF_LATEST_QUEUE_NAME
  const historyQueueName = L2_LISTENER_VRF_HISTORY_QUEUE_NAME
  const processEventQueueName = L2_LISTENER_VRF_PROCESS_EVENT_QUEUE_NAME
  const workerQueueName = L2_WORKER_VRF_QUEUE_NAME
