...
func TestUpdateProofs(t *testing.T) {
	...
	result, err := db.QueryRows[Proof](ctx, "SELECT config_id, round, proof FROM proofs WHERE config_id IN ("+strconv.Itoa(int(tmpConfigs[0].ID))+", "+strconv.Itoa(int(tmpConfigs[1].ID))+")", nil)
	if err != nil {
