    ...
    if (accountLists.length > 0) {
      for (const accountInfo of accountLists) {
        try {
          ...
          const alarm_amount = await this.accountBalanceRepository.getBalance(accountInfo.address);
          if (balance && alarm_amount) {
            // if value is '0' mean disable alarm
            // if value is '1'~'n' mean number of balance_alarm_amount
            if (Math.floor(balance) < alarm_amount && alarm_amount !== 0) {
              this.sendToSlackLowBalance(accountInfo, balance, alarm_amount);
            }
          }
        } catch (e) {
          console.log(e);
