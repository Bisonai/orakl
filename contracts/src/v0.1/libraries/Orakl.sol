...
library Orakl {
  ...
  function initialize(
    ...
    address callbackAddr,
    bytes4 callbackFunc
  ) internal /*pure*/ view returns (Orakl.Request memory) {
    Buffer.init(self.buf, defaultBufferSize);
    self.id = jobId;
    self.callbackAddr = callbackAddr;
    self.callbackFunc = callbackFunc;
    console.log('initialize self.buf.capacity %s', self.buf.capacity);
    console.log('initialize self.buf.buf.length %s', self.buf.buf.length);
    return self;
  }
   ...
   */
  function setBuffer(Request memory _request, bytes memory _data) internal /*pure*/ view {
  ...
  function add(
    ...
    console.log('add self.buf %s', self.buf.buf.length);
    console.log('add key %s ', key);
    console.log('add value %s ', value);
