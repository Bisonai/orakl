...
// https://github.com/smartcontractkit/chainlink/blob/develop/contracts/src/v0.6/DeviationFlaggingValidator.sol
 ...
 * flag contract.
 */
contract DeviationFlaggingValidator is ConfirmedOwner, AggregatorValidatorInterface {
    int public constant THRESHOLD_MULTIPLIER = 100_000;
    int256 private constant INT256_MIN = -2 ** 255;

    uint32 public flaggingThreshold;
    FlagsInterface public flags;

    event FlaggingThresholdUpdated(uint24 indexed previous, uint24 indexed current);
     ...
     */
    constructor(address _flags, uint24 _flaggingThreshold) ConfirmedOwner(msg.sender) {
    ...
    function isValid(
        ...
        int256 change = _previousAnswer - _answer;
        int256 ratioNumerator = change * THRESHOLD_MULTIPLIER;
        int256 ratio = ratioNumerator / _previousAnswer;
        uint256 absRatio = abs(ratio);

        return absRatio <= flaggingThreshold;
    }
     ...
     * change compared to the previous price.
     */
    function setFlaggingThreshold(uint24 _flaggingThreshold) public onlyOwner {
     ...
     * @param _flags sets the address of the flags contract
     */
    function setFlagsAddress(address _flags) public onlyOwner {
    ...
    function abs(int256 value) private pure returns (uint256) {
        require(value != INT256_MIN, "Cannot apply abs");
        if (value >= 0) {
