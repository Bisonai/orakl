...
async function main() {
  ...
  const subId = await Prepayment.createAccount()
  console.log(subId)
  if (listen) {
    Prepayment.once('AccountCreated', async (subId, owner) => {
  ...
  await Prepayment.addConsumer(1, VRFConsumerMock.address)
  if (listen) {
  ...
  await Prepayment.deposit(1, { value: parseEther(10) })
  const [owner, addr1, addr2] = await ethers.getSigners()
  await owner.sendTransaction({ to: VRFCoordinator.address, value: parseEther(1) })
